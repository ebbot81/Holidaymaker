...
public class Program {
    ...
    int menu;
        ...
        System.out.println("[3] HITTA DIN DRÖMRESA");
    ...
    public void bookingMenu() {
        ...
        do {
            System.out.println("\n         HOLIDAYMAKER");
            System.out.println("-----------------------------");
            System.out.println("NEDAN FÖLJER OLIKA SÖKKRITERIER FÖR ATT HITTA JUST DIN DRÖMRESA\n");
            ...
            System.out.println("[5] BOKA DIN RESA HÄR");
    ...
    private ArrayList<Room> searchOccupiedRoom() {
        String query = "SELECT r.id_room, r.location, rs.name 'room_size_name', rs.number_of_beds FROM rooms AS r INNER JOIN room_size AS rs ON r.room_size_id = rs.id WHERE room_availability = 0";
    ...
    private Room getRoomForCancellation(int numberOfBeds){
        ...
        System.out.println("Ange rumsid:et du vill ta bort");
        int id = Integer.parseInt(scanner.nextLine());

        for(Room room : vacantRooms) {
            if(room.getId() == id) {
                vacantRooms.remove(id);
            }
        }
    ...
    private void bookRoom() {
        ...
        if (endDate.before(startDate)) {
            System.out.println("Du kan inte resa hem innan du rest iväg :)");
            return;
        }
    ...
    private void cancelRoom() {
        ...
        Room room = getRoomForCancellation(numberOfTravelers);
